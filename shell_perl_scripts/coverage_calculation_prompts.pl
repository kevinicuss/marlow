...
# This script will search a directory for .txt files 
#and setup command line input prompt to find coverage_calculation per base in each genoe file

# coverage_calculation_prompts
...
# sample run: perl coverage_calculation.pl -b /scratch.global/props027/base_coverage -g ${genome} --output <base_coverage_prompts.csv> --help <help on usage of script>
...
foreach my $files (@files_to_read) {
    my $total = length($files[]
    print "$files\n";
    print $output_fh "perl coverage_calculation.pl -b /scratch.global/props027/base_coverage -g sum(len($file[])) -h help
}
